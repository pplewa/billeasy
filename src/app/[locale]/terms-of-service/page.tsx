    ...
    <div className="container mx-auto px-4 py-12 max-w-4xl">
      ...
      <div className="prose prose-lg max-w-none">
        ...
        <ul className="list-disc pl-6 my-4 space-y-2">
          {(
            (messages.termsOfService as unknown as { serviceDescriptionItems: [] })
              .serviceDescriptionItems as unknown as string[]
          ).map((item: string) => (
            <li key={item}>{item}</li>
          ))}
        </ul>
        ...
        <ul className="list-disc pl-6 my-4 space-y-2">
          {(
            (messages.termsOfService as unknown as { userObligationsItems: [] })
              .userObligationsItems as unknown as string[]
          ).map((item: string) => (
            <li key={item}>{item}</li>
          ))}
        </ul>
        ...
        <ul className="list-disc pl-6 my-4 space-y-2">
          {(
            (messages.termsOfService as unknown as { dataAndContentResponsibilities: [] })
              .dataAndContentResponsibilities as unknown as string[]
          ).map((item: string) => (
            <li key={item}>{item}</li>
          ))}
        </ul>
        ...
        <ul className="list-disc pl-6 my-4 space-y-2">
          {(
            (messages.termsOfService as unknown as { limitationOfLiabilityItems: [] })
              .limitationOfLiabilityItems as unknown as string[]
          ).map((item: string) => (
            <li key={item}>{item}</li>
          ))}
        </ul>
