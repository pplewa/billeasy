...
export function processInvoice(invoiceData: unknown): Invoice {
  ...
  const globalTax = data.details?.tax || data.details?.taxDetails || { amount: 0, amountType: 'percentage' };
  const globalDiscount = data.details?.discount || data.details?.discountDetails || { amount: 0, amountType: 'percentage' };
  const shipping = data.details?.shipping || data.details?.shippingDetails || { cost: 0, costType: 'fixed' };
...
const formInvoiceDetailsSchema = z.object({
  ...
  shipping: baseAmountSchema.extend({
    cost: z.number(),
